...
function analyzeString(value) {
  ...
  const isPalindrome =
    cleanValue.toLowerCase().replace(/\s+/g, "") ===
    cleanValue.toLowerCase().replace(/\s+/g, "").split("").reverse().join("");
...
app.get("/strings", (req, res) => {
  ...
  try {
    ...
    if (min_length) {
      results = results.filter(
        (item) => item.properties.length >= parseInt(min_length)
      );
    }
    if (max_length) {
      results = results.filter(
        (item) => item.properties.length <= parseInt(max_length)
      );
    }
    if (word_count) {
      results = results.filter(
...
app.get("/strings/filter-by-natural-language", (req, res) => {
  ...
  if (filters.word_count)
    filtered = filtered.filter(
      (i) => i.properties.word_count === filters.word_count
    );
  if (filters.is_palindrome)
    filtered = filtered.filter((i) => i.properties.is_palindrome);
  if (filters.min_length)
    filtered = filtered.filter(
      (i) => i.properties.length >= filters.min_length
    );
  if (filters.contains_character)
    filtered = filtered.filter((i) =>
      i.value.includes(filters.contains_character)
    );
...
app.delete("/strings/:string_value", (req, res) => {
  const { string_value } = req.params;
  const index = analyzedStrings.findIndex(
